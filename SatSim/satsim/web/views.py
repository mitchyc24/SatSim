...
def run_simulation(scenario_id):
    ...
    if plots_error and run.plot_files:
        flash(
            "matplotlib is unavailable in this environment (%s), so the "
            "figures below were drawn by SatSim's built-in SVG renderer. "
            "The analysis results are unaffected." % plots_error,
            "warning",
        )
    elif plots_error:
        flash(
            "Metrics and pass schedule were saved, but no figures could be "
            "rendered (%s). The analysis results below are unaffected."
            % plots_error,
            "warning",
        )
